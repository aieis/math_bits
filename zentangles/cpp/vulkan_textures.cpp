...
bool LoadTextureFromFile(VkDevice device, VkPhysicalDevice physicalDevice, VkAllocationCallbacks* allocator, VkCommandPool command_pool, VkQueue queue, const char* filename, TextureData* tex_data)
{
    tex_data->Channels = 4;
    cv::Mat im = cv::imread(filename);
    cv::cvtColor(im, im, cv::COLOR_BGR2BGRA);
    unsigned char* image_data  = im.data;
    tex_data->Width = im.cols;
    tex_data->Height = im.rows;
    ...
    // Create the Vulkan image.
    {
        ...
        alloc_info.memoryTypeIndex = findMemoryType(physicalDevice, req.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
void RemoveTexture(VkDevice g_Device, TextureData* tex_data)
{
